package net.mcreator.worldrender.procedures;
...
public class TSTProcedure {
	...
	private static boolean begin(VertexFormat.Mode mode, VertexFormat format, boolean update) {
		if (TSTProcedure.bufferBuilder == null || !TSTProcedure.bufferBuilder.building()) {
			if (update)
				clear();
			if (TSTProcedure.vertexBuffer == null) {
				if (format == DefaultVertexFormat.POSITION_COLOR) {
					TSTProcedure.mode = mode;
					TSTProcedure.format = format;
					TSTProcedure.bufferBuilder = Tesselator.getInstance().getBuilder();
					TSTProcedure.bufferBuilder.begin(mode, DefaultVertexFormat.POSITION_COLOR);
					return true;
				} else if (format == DefaultVertexFormat.POSITION_TEX_COLOR) {
					TSTProcedure.mode = mode;
					TSTProcedure.format = format;
					TSTProcedure.bufferBuilder = Tesselator.getInstance().getBuilder();
					TSTProcedure.bufferBuilder.begin(mode, DefaultVertexFormat.POSITION_TEX_COLOR);
					return true;
				}
			}
		}
	...
	private static void system(boolean worldCoordinate) {
		TSTProcedure.worldCoordinate = worldCoordinate;
	}

	private static boolean target(int targetStage) {
		if (targetStage == currentStage) {
			TSTProcedure.targetStage = targetStage;
			return true;
		}
		return false;
	}
	...
	private static void execute(@Nullable Event event, LevelAccessor world) {
		if (world instanceof ClientLevel) {
			for (Entity entityiterator : ((ClientLevel) world).entitiesForRendering()) {
				renderBlock(Blocks.TNT.defaultBlockState(), (entityiterator.getX()), (entityiterator.getY() + 2), (entityiterator.getZ()), 0, 0, 0, 1, false);
			}
		}
	}
}
